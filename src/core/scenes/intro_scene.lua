...
function intro:move()
  ...
  self.moving_circle.x = math.sin(self.time) * 0.5 + 0.5
...
function love.keypressed(button, player)
  print("keypressed", button)

  if button == 'd' then
    intro.player_circle.x = intro.player_circle.x + 10
    intro.video:setDesiredFPS(30)
    return true
  end

  if button == 'a' then
    intro.player_circle.x = intro.player_circle.x - 10
    intro.video:setDesiredFPS(120)
    return true
  end
